...
import { getCameras, login, getToken, queryAttrs } from '../../aqara.js';
...
async function main() {
  ...
  // the preferred port — or any in the block — is already taken by e.g. go2rtc).
  ...
  const activeBridges: Array<{ bridge: AqaraCameraBridge; slug: string; did: string; port: number }> = [];
  ...
  for (const cam of cameras) {
    ...
    try {
      await bridge.start();
      activeBridges.push({ bridge, slug: streamSlug, did: cam.did, port: rtspPort });
    } catch (err: any) {
