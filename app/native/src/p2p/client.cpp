...
struct P2PLoginPayload {
    std::string timestamp{};
    std::string app_sign{};
    std::string app_public_key{};
};

struct P2PQualityPayload {
    int channel{0};
};

struct P2PPTZPayload {
    int action{0};
    int speed{0};
};
...
void P2PClient::send_login_if_due(int64_t min_interval_ms) {
    ...
    P2PLoginPayload payload{
        .timestamp = config_.sign_time.empty() ? std::to_string(now) : config_.sign_time,
        .app_sign = config_.app_sign,
        .app_public_key = config_.app_pub_hex,
    };
    std::string login_json = glz::write_json(payload).value_or("");
    std::cout << "[P2P-Native] Sending 0x1000 Login for " << config_.did << std::endl;
    auto login_frame = PPCSCipher::build_lumi_frame(
        LumiCmdType::LOGIN_REQ, reinterpret_cast<const uint8_t*>(login_json.data()), login_json.length(), cmd_seq_++);
...
void P2PClient::set_quality(int channel) {
    P2PQualityPayload payload{.channel = channel};
    std::string json = glz::write_json(payload).value_or("");
    auto frame = PPCSCipher::build_lumi_frame(LumiCmdType::SET_QUALITY, reinterpret_cast<const uint8_t*>(json.data()),
                                              json.length(), cmd_seq_++);
    send_enc_drw(0, ch0_seq_++, frame.data(), frame.size());
}

void P2PClient::ptz(int action, int speed) {
    P2PPTZPayload payload{.action = action, .speed = speed};
    std::string json = glz::write_json(payload).value_or("");
    auto frame = PPCSCipher::build_lumi_frame(LumiCmdType::PTZ_OR_TALK_START,
                                              reinterpret_cast<const uint8_t*>(json.data()), json.length(), cmd_seq_++);
    send_enc_drw(0, ch0_seq_++, frame.data(), frame.size());
}
...
void P2PClient::start_talkback() {
    ...
    talk_seq_ = 0;
    talk_frames_sent_ = 0;
    auto frame = PPCSCipher::build_lumi_frame(LumiCmdType::PTZ_OR_TALK_START, nullptr, 0, cmd_seq_++);
    send_enc_drw(0, ch0_seq_++, frame.data(), frame.size());
}
...
void P2PClient::stop_talkback() {
    ...
    auto frame = PPCSCipher::build_lumi_frame(LumiCmdType::TALK_STOP, nullptr, 0, cmd_seq_++);
    send_enc_drw(0, ch0_seq_++, frame.data(), frame.size());
}
...
void P2PClient::discovery_loop() {
    ...
    auto query_pkt = PPCSCipher::build_pppp(PpcsMsgType::QUERY, req20, 36);
    PPCSCipher::encrypt(ppcs_key_.data(), ppcs_key_.size(), query_pkt.data(), query_pkt.size());

    auto hello_pkt = PPCSCipher::build_pppp(PpcsMsgType::HELLO);
    ...
    uint8_t lan_bcast[4] = {0xF1, static_cast<uint8_t>(PpcsMsgType::LAN_BROADCAST), 0x00, 0x00};
...
void P2PClient::handle_packet(const uint8_t* data, size_t len, const sockaddr_in& src) {
    ...
    if ((msg_type == 0x40 || msg_type == 0x21) && payload_len >= 8) {
        ...
        auto rdy_pkt = PPCSCipher::build_pppp(PpcsMsgType::READY, punch_buf_.data(), punch_buf_.size());
        PPCSCipher::encrypt(ppcs_key_.data(), ppcs_key_.size(), rdy_pkt.data(), rdy_pkt.size());
        send_raw_packet(rdy_pkt.data(), rdy_pkt.size(), src);
    } else if (msg_type == static_cast<uint8_t>(PpcsMsgType::READY)) {
    ...
    } else if (msg_type == static_cast<uint8_t>(PpcsMsgType::READY_ACK)) {
        ...
        send_login_if_due();
    } else if (msg_type == static_cast<uint8_t>(PpcsMsgType::ALIVE)) {
        // ALIVE -> reply ALIVE_ACK
        auto ack = PPCSCipher::build_pppp(PpcsMsgType::ALIVE_ACK);
        PPCSCipher::encrypt(ppcs_key_.data(), ppcs_key_.size(), ack.data(), ack.size());
        send_raw_packet(ack.data(), ack.size(), src);
    } else if ((msg_type == static_cast<uint8_t>(PpcsMsgType::DRW) ||
                msg_type == static_cast<uint8_t>(PpcsMsgType::DRW_EXTRA) ||
                msg_type == static_cast<uint8_t>(PpcsMsgType::ALT_DRW)) &&
               payload_len >= 4 && payload[0] == static_cast<uint8_t>(PpcsMsgType::DRW_ACK)) {
...
void P2PClient::dispatch_channel0(uint32_t type, const uint8_t* body, size_t body_len) {
    ...
    if (type == static_cast<uint32_t>(LumiCmdType::LOGIN_RESP)) {
        ...
        if (!session_started_.exchange(true)) {
            ...
            auto stream_frame = PPCSCipher::build_lumi_frame(LumiCmdType::STREAM_START_REQ, nullptr, 0, cmd_seq_++);
            send_enc_drw(3, ch3_seq_++, stream_frame.data(), stream_frame.size());

            auto session_frame = PPCSCipher::build_lumi_frame(LumiCmdType::SESSION_START_REQ, nullptr, 0, cmd_seq_++);
            send_enc_drw(0, ch0_seq_++, session_frame.data(), session_frame.size());
        }
    } else if (type == static_cast<uint32_t>(LumiCmdType::SESSION_START_RESP)) {
        if (!session_ready_) {
            session_ready_ = true;
            std::cout << "[P2P-Native] 0x1003 Session Ready for " << config_.did << ", requesting keyframe (0x1018)"
                      << std::endl;
            session_ready_since_ms_ = current_time_ms();
            request_keyframe();
    ...
    } else if (type == static_cast<uint32_t>(LumiCmdType::TALKBACK_RESP)) {
        ...
        if (is_connected_) {
            ...
            if (!session_started_) {
                ...
                if (video_at == 0 && ready_at > 0 && now - ready_at > 4000 &&
                    ...
                    auto stream_frame =
                        PPCSCipher::build_lumi_frame(LumiCmdType::STREAM_START_REQ, nullptr, 0, cmd_seq_++);
                    send_enc_drw(3, ch3_seq_++, stream_frame.data(), stream_frame.size());
                    auto session_frame =
                        PPCSCipher::build_lumi_frame(LumiCmdType::SESSION_START_REQ, nullptr, 0, cmd_seq_++);
                    send_enc_drw(0, ch0_seq_++, session_frame.data(), session_frame.size());
